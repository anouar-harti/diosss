  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-20">
         <div className="flex items-center gap-3 mb-4">
            <button onClick={onBack} className="bg-white dark:bg-slate-800 p-2 rounded-full shadow-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">
                <ChevronLeft size={20} />
            </button>
            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Mis Tareas Pendientes</h2>
        </div>

        <div className="space-y-4">
            {tasks.length === 0 ? (
                <div className="text-center py-10 text-slate-400 dark:text-slate-500 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-600">
                ...
                tasks.map(task => (
                    ...
                    >
                        <div className="flex justify-between items-start mb-2">
                            <h3 className={`font-bold text-lg ${task.isCompleted ? 'text-green-800' : 'text-slate-800 dark:text-slate-100'}`}>
                                {task.title}
                            </h3>
                            <button 
                                ...
                                className={`p-2 rounded-full transition-colors ${
                                    ...
                                    : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 hover:bg-green-100 hover:text-green-600'
                                }`}
                            >
                        ...
                        <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 mb-3">
                            <MapPin size={14} /> {task.location}
                        </div>
                        
                        <p className="text-slate-600 dark:text-slate-300 text-sm mb-4 bg-white dark:bg-slate-800/50 p-2 rounded-lg">
                            {task.description}
                        </p>
...
const App: React.FC = () => {
  ...
  const createPDFDocument = async () => {
    ...
    // Internal helper to squash huge png strings (like signatures and photos) to tiny JPEGs
    ...
    const clientSignatureCompressed = signature ? await compressImage(signature, 0.4) : null;
    
    // Compress BEFORE and AFTER photos heavily since they can be huge
    const photoBeforeCompressed = photoBefore ? await compressImage(photoBefore, 0.4) : null;
    const photoAfterCompressed = photoAfter ? await compressImage(photoAfter, 0.4) : null;
            ...
            const compressedLogo = await new Promise<string>((resolve) => {
                ...
                img.onload = () => {
                    ...
                    if(ctx){
                        ...
                        resolve(COMPANY_LOGO_URL);
                    }
                };
                img.onerror = () => resolve(COMPANY_LOGO_URL);
                img.src = COMPANY_LOGO_URL;
            });

            doc.addImage(compressedLogo, 'JPEG', 15, 5, 35, 35); // slightly bigger to look nice
            titleX = 55;
        } catch (e) {
    ...
    if (workerSignatureCompressed) {
        doc.addImage(workerSignatureCompressed, 'JPEG', 25, boxY + 10, 60, 25);
    }
    doc.setFontSize(10);
    doc.setTextColor(0,0,0);
    ...
    if (clientSignatureCompressed) {
        doc.addImage(clientSignatureCompressed, 'JPEG', 120, boxY + 10, 60, 25);
    }
    doc.setFontSize(10);
    doc.setTextColor(0,0,0);
    ...
    if (photoBeforeCompressed || photoAfterCompressed) {
        doc.addPage();
        ...
        if (photoBeforeCompressed) {
            ...
            try {
                ...
                doc.addImage(photoBeforeCompressed, 'JPEG', 15, py + 5, 180, 100, undefined, 'FAST');
                py += 115;
            } catch (e) {
        ...
        if (photoAfterCompressed) {
            ...
            try {
                doc.addImage(photoAfterCompressed, 'JPEG', 15, py + 5, 180, 100, undefined, 'FAST');
            } catch (e) {
      ...
      <div className="flex flex-col items-center justify-center min-h-[80vh] px-4">
          <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-xl w-full max-w-md border border-slate-100 dark:border-slate-700">
              <div className="flex flex-col items-center mb-6">
                ...
                <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100">HARTI ELECTROCOOL</h1>
                <p className="text-sm text-slate-400 dark:text-slate-500">Acceso a ClimaTrack Pro</p>
              </div>

              <form onSubmit={handleLogin} className="space-y-4">
                  <div>
                      <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase mb-1">Usuario</label>
                      <div className="relative">
                          ...
                          <input 
                            ...
                            className="w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all text-slate-800 dark:text-slate-100"
                            placeholder="Ingrese su usuario"
                          />
                      </div>
                  </div>
                  <div>
                      <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase mb-1">Contraseña</label>
                      <div className="relative">
  ...
  const renderDashboard = () => (
      <div className="space-y-6 animate-fadeIn">
          {deferredPrompt && (
              <div className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 p-1 rounded-2xl shadow-lg animate-pulse">
                <button 
                    ...
                    className="w-full bg-white dark:bg-slate-800 rounded-xl p-4 flex items-center justify-between group"
                >
                    <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                            <p className="font-bold text-slate-800 dark:text-slate-100">Instalar Aplicación</p>
                            <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">Añadir a la pantalla de inicio</p>
                        </div>
                    </div>
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-600">
              <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-1">Hola, {currentUser?.fullName}</h2>
              <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 text-sm">¿Qué te gustaría hacer hoy?</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                  <div className="bg-white dark:bg-slate-800/20 p-3 rounded-full">
                    <FileText size={32} />
                  </div>
              ...
              {currentUser?.role === 'WORKER' && (
                  <button 
                    ...
                    className="bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 text-slate-700 dark:text-slate-200 p-6 rounded-2xl shadow-sm hover:border-blue-200 transition-all active:scale-95 flex flex-col items-center justify-center gap-3"
                  >
  ...
  const renderAdminUsers = () => (
      <div className="space-y-6 animate-fadeIn">
          <div className="flex items-center gap-3 mb-4">
              <button onClick={() => setView(AppView.DASHBOARD)} className="bg-white dark:bg-slate-800 p-2 rounded-full shadow-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">
                  <ChevronLeft size={20} />
              </button>
              <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Crear Nuevo Usuario</h2>
          </div>

          <form onSubmit={handleCreateUser} className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-600 space-y-4">
              <div>
                  <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase mb-1">Nombre Completo</label>
                  <input 
                    type="text" 
                    value={newUserFullname}
                    onChange={(e) => setNewUserFullname(e.target.value)}
                    className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-slate-800 dark:text-slate-100"
                    placeholder="Ej. Juan Pérez"
                  />
              </div>
              <div>
                  <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase mb-1">Nombre de Usuario (Login)</label>
                  <input 
                    type="text" 
                    value={newUserUser}
          ...
          <div className="bg-slate-100 dark:bg-slate-800 p-4 rounded-xl">
              <h3 className="text-sm font-bold text-slate-600 dark:text-slate-300 mb-2 flex items-center gap-2"><Users size={16}/> Usuarios Existentes</h3>
              <ul className="space-y-2">
                  {adminUsersList.map((u, i) => (
                      <li key={u.username} className="bg-white dark:bg-slate-800 p-3 rounded-lg text-sm flex justify-between items-center shadow-sm">
                          <div className="flex flex-col">
                              <span className="font-medium text-slate-800 dark:text-slate-100">{u.fullName}</span>
                              <span className="text-slate-400 dark:text-slate-500 text-xs">({u.username})</span>
                          </div>
                          <button 
                             type="button"
                  ...
                  {adminUsersList.length === 0 && <p className="text-xs text-slate-400 dark:text-slate-500 italic">No hay trabajadores creados.</p>}
              </ul>
          </div>
      </div>
  );
      ...
      return (
        <div className="space-y-6 animate-fadeIn pb-20">
            <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Asignar y Supervisar</h2>
            </div>

            <form onSubmit={handleCreateTask} className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-600 space-y-4">
                <h3 className="text-sm font-bold text-purple-700 flex items-center gap-2 mb-2">
                    <Briefcase size={16} /> Crear Nueva Asignación
                </h3>
                <div>
                    <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase mb-1">Título del Trabajo</label>
                    <input 
                        type="text" 
                        value={newTaskTitle}
                ...
                <div>
                    <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase mb-1">Ubicación / Dirección</label>
                    <input 
                        type="text" 
                        value={newTaskLoc}
                ...
                <div>
                    <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase mb-1">Descripción</label>
                    <textarea 
                        value={newTaskDesc}
                        onChange={(e) => setNewTaskDesc(e.target.value)}
                        className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 h-24 text-slate-800 dark:text-slate-100"
                        placeholder="Detalles del trabajo..."
                    />
                </div>
                <div>
                    <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase mb-1">Asignar a Trabajador</label>
                    <select 
            ...
            <div className="space-y-3">
                ...
                {adminTasksList.length === 0 ? (
                    <div className="text-center py-6 text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-600 border-dashed">
                        No hay trabajos activos asignados.
                    </div>
                ) : (
                    adminTasksList.slice().reverse().map(task => (
                        <div key={task.id} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-600 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div className="flex-1">
                                ...
                                <div className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500 flex flex-wrap gap-3">
                                    <span className="flex items-center gap-1">
                                        <User size={12} /> Asignado a: <strong className="text-slate-700 dark:text-slate-200">{users.find(u => u.username === task.assignedTo)?.fullName || task.assignedTo}</strong>
                                    </span>
                                    <span className="flex items-center gap-1">
                            ...
                            <div className="flex items-center gap-3">
                                <div className={`p-2 rounded-full ${task.isCompleted ? 'bg-green-100 text-green-600' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500'}`}>
    ...
    <div className="space-y-6 animate-fadeIn">
        <div className="flex items-center gap-3 mb-2">
            <button onClick={() => {
                ...
                setIsPaused(false);
                setPauseStartTime(null);
            }} className="bg-white dark:bg-slate-800 p-2 rounded-full shadow-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">
                <ChevronLeft size={20} />
            </button>
        ...
        />
        {step === AppStep.IDLE && (
            <div className="text-center text-sm text-slate-400 dark:text-slate-500 mt-4">
  ...
  const renderDetails = () => (
    <div className="space-y-6 animate-fadeIn pb-20">
        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2 border-b border-slate-200 dark:border-slate-600 pb-2">
        ...
        <div className="bg-slate-100 dark:bg-slate-800 p-4 rounded-xl space-y-3">
             ...
             <input 
                ...
                className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100"
             />
        </div>
        ...
        <div className="space-y-2">
            <div className="flex justify-between items-center">
                <label className="text-sm font-medium text-slate-600 dark:text-slate-300">Descripción del Trabajo</label>
                <button 
        ...
        <div className="bg-orange-50 p-4 rounded-xl border border-orange-100 space-y-2">
             ...
             <div className="relative">
                <input 
                    ...
                    placeholder="0.00"
                    className="w-full p-3 pl-8 border border-orange-200 rounded-lg focus:ring-2 focus:ring-orange-500 outline-none bg-white dark:bg-slate-800 text-lg font-bold text-slate-700 dark:text-slate-200"
                />
  ...
  const renderPhotos = () => (
      <div className="space-y-6 animate-fadeIn pb-20">
          <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 border-b border-slate-200 dark:border-slate-600 pb-2 flex items-center gap-2">
              <Camera size={24} className="text-blue-600" /> Documentación Visual
          </h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">Tome fotos del antes y el después para adjuntarlas al parte de trabajo.</p>

          <div className="space-y-4">
              {/* Photo Before */}
              <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-600 shadow-sm space-y-3">
                  <h3 className="font-semibold text-slate-700 dark:text-slate-200 text-sm uppercase flex items-center gap-1">
                  ...
                  {photoBefore ? (
                      <div className="relative rounded-lg overflow-hidden border border-slate-200 dark:border-slate-600">
                          <img src={photoBefore} alt="Antes" className="w-full h-auto max-h-64 object-contain bg-slate-100 dark:bg-slate-800" />
                      ...
                      <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-300 rounded-xl cursor-pointer bg-slate-50 dark:bg-slate-900 hover:bg-slate-100 dark:bg-slate-800 transition-colors">
                          <div className="flex flex-col items-center justify-center pt-5 pb-6">
                              <Camera className="w-8 h-8 text-slate-400 dark:text-slate-500 mb-2" />
                              <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">Tomar o subir foto</p>
                          </div>
                          <input type="file" accept="image/*" className="hidden text-slate-800 dark:text-slate-100 dark:bg-slate-900" onChange={(e) => handlePhotoUpload(e, setPhotoBefore)} />
                      </label>
                  )}
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-600 shadow-sm space-y-3">
                  ...
                  {photoAfter ? (
                      <div className="relative rounded-lg overflow-hidden border border-slate-200 dark:border-slate-600">
                          <img src={photoAfter} alt="Después" className="w-full h-auto max-h-64 object-contain bg-slate-100 dark:bg-slate-800" />
                      ...
                      <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-300 rounded-xl cursor-pointer bg-slate-50 dark:bg-slate-900 hover:bg-slate-100 dark:bg-slate-800 transition-colors">
                          ...
                          <input type="file" accept="image/*" className="hidden text-slate-800 dark:text-slate-100 dark:bg-slate-900" onChange={(e) => handlePhotoUpload(e, setPhotoAfter)} />
                      </label>
                  )}
              </div>
          </div>
          ...
          <div className="flex justify-between pt-4">
              <button 
                  ...
                  className="flex items-center gap-2 text-slate-600 dark:text-slate-300 px-4 py-3 font-medium hover:bg-slate-100 dark:bg-slate-800 rounded-xl"
              >
  ...
  const renderSignature = () => (
    <div className="space-y-8 animate-fadeIn pb-20">
        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 border-b border-slate-200 dark:border-slate-600 pb-2">Firmas y Conformidad</h2>
        
        <div className="space-y-2">
             <h3 className="font-semibold text-slate-700 dark:text-slate-200 text-sm uppercase tracking-wide">Firma del Técnico ({workerName})</h3>
             <SignaturePad onSave={setWorkerSignature} title="Firma del Técnico" />
        </div>

        <div className="space-y-2">
            <h3 className="font-semibold text-slate-700 dark:text-slate-200 text-sm uppercase tracking-wide">Firma del Cliente ({clientName})</h3>
  ...
  const renderReview = () => (
      <div className="space-y-6 animate-fadeIn">
          <div className="text-center space-y-2 py-6">
              ...
              <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">¡Trabajo Completado!</h2>
              <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 max-w-xs mx-auto">
          ...
          <div className="grid grid-cols-1 gap-4">
              <button 
                ...
                className="w-full flex items-center justify-center gap-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 py-4 rounded-xl font-bold hover:bg-slate-200 dark:bg-slate-700 transition-all shadow-sm active:scale-95 border border-slate-200 dark:border-slate-600"
              >
                ...
                onClick={async () => {
                    ...
                    try {
                        ...
                        if (success) {
                            ...
                            alert("Se agotó el tiempo de subida en Firebase Storage. Cambia las reglas tal como se indica en la Opción B.");
                            setIsEnhancing(false);
                        }
  ...
  return (
    <>
      <AnimatePresence>
        {showSplash && (
          <motion.div
            ...
            className="fixed inset-0 z-[200] bg-white dark:bg-slate-800 flex flex-col items-center justify-center pointer-events-none shadow-[-20px_0_40px_rgba(0,0,0,0.15)]"
          >
             <div className="flex flex-col items-center">
                 ...
                 <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">HARTI</h1>
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col font-sans">
        ...
        {showHeader && (
        <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-600 sticky top-0 z-50 shadow-sm">
          <div className="max-w-3xl mx-auto px-4 h-16 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div className="leading-none">
                  <h1 className="text-sm font-bold text-slate-800 dark:text-slate-100 tracking-tight">HARTI</h1>
            ...
            <div className="flex items-center gap-4">
                        ...
                        {[AppStep.IDLE, AppStep.WORKING, AppStep.DETAILS, AppStep.PHOTOS, AppStep.SIGNATURE, AppStep.REVIEW].map((s, i) => (
                            <div 
                                ...
                                className={`h-2 w-2 rounded-full transition-colors ${
                                    Object.values(AppStep).indexOf(step) >= i ? 'bg-blue-600' : 'bg-slate-200 dark:bg-slate-700'
                                }`}
                            />
                        ))}
                    </div>
                 ...
                 <div className="flex items-center gap-3">
                     <button title="Forzar Actualización" onClick={() => { if ('serviceWorker' in navigator) { navigator.serviceWorker.getRegistrations().then(r => r.forEach(reg => reg.unregister())); } window.location.href = window.location.pathname + '?v=' + new Date().getTime(); }} className="text-slate-400 dark:text-slate-500 hover:text-green-500 transition-colors">
                         <span className="text-[10px] font-bold">V3</span>
                     </button>
                     <button onClick={() => setIsDarkMode(!isDarkMode)} className="text-slate-400 dark:text-slate-500 hover:text-blue-500 transition-colors">
                         {isDarkMode ? <Sun size={20} /> : <Moon size={20} />}
                     </button>
                     <button onClick={() => setShowLogoutConfirm(true)} className="text-slate-400 dark:text-slate-500 hover:text-red-500 transition-colors">
                         <LogOut size={20} />
                     </button>
                 </div>
            </div>
      ...
      <footer className="p-4 text-center text-slate-400 dark:text-slate-500 text-xs">
              ...
              <div className="w-full max-w-5xl mx-auto h-full bg-white dark:bg-slate-800 rounded-2xl overflow-hidden flex flex-col shadow-2xl">
                  <div className="flex justify-between items-center p-4 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-600">
                      ...
                      <button 
                        ...
                        className="text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-red-500 hover:bg-red-50 p-2 rounded-lg transition-colors"
                      >
                          <X size={24} />
                      </button>
                  </div>
                  <div className="flex-1 w-full bg-slate-200 dark:bg-slate-700 relative flex flex-col items-center justify-center p-4">
      ...
      {showLogoutConfirm && (
          <div className="fixed inset-0 z-[100] bg-slate-900 bg-opacity-60 flex items-center justify-center p-4 backdrop-blur-sm animate-fadeIn">
              <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl relative border border-slate-100 dark:border-slate-700">
                  <div className="mx-auto w-12 h-12 bg-red-100 text-red-600 rounded-full flex items-center justify-center mb-4">
                      <LogOut size={24} />
                  </div>
                  <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-2 text-center">¿Cerrar sesión?</h3>
                  {step !== AppStep.IDLE && step !== AppStep.REVIEW ? (
                      <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 mb-6 text-sm text-center">
                  ...
                  <div className="flex gap-3">
                      <button 
                          ...
                          className="flex-1 px-4 py-3 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-bold rounded-xl hover:bg-slate-200 dark:bg-slate-700 transition-colors"
                      >
                          Cancelar
                      </button>
                      <button 
